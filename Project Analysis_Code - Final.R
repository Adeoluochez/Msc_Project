...
library(tidyverse) # for data wrangling, data manipulation
...
#****RENAMING THE VARIABLES IN BOTH DATA SET******

# renaming variables from the schools data set
...
# renaming variables from the students data set
...
my_data <- stu_data_2 %>% inner_join(sch_data_2, by = "CNTSCHID")
...
View(naniar::miss_var_summary(my_data))
...
naniar::vis_miss(my_data_2)

# Explore patterns of missing data using upset plot

naniar::gg_miss_upset(my_data_2)

# removing NAs

my_data_3 <-my_data_2 %>% na.omit(my_data_2)


#****RECODING THE NEW DATA SET "my_data_3******
#
...
school_proportion <- my_data_3 %>%
  group_by(sch_type) %>%
  summarise(count = n_distinct(CNTSCHID)) %>%
...
additional_lesson <- my_data_3 %>%
  group_by(sch_offer_aditional_lessons) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
Lesson_Enrichment <- my_data_3 %>%
  group_by(mathsLesson_enrichment) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
Lesson_Remedial <- my_data_3 %>%
  group_by(mathsessons_remedial) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
Lesson_Others <- my_data_3 %>%
  group_by(mathslessons_others) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
schtype_enrichment <- my_data_3 %>%
  group_by(sch_type, mathsLesson_enrichment) %>%
  summarise(count = n_distinct(CNTSCHID)) %>%
  mutate(percentage = count / sum(count) * 100) %>%
  pivot_wider(names_from = mathsLesson_enrichment, values_from = c(count, percentage), values_fill = 0)

print(schtype_enrichment)

#Remedial

schtype_remedial <- my_data_3 %>%
  group_by(sch_type, mathsessons_remedial) %>%
  summarise(count = n_distinct(CNTSCHID)) %>%
  mutate(percentage = count / sum(count) * 100) %>%
  pivot_wider(names_from = mathsessons_remedial, values_from = c(count, percentage), values_fill = 0)

print(schtype_remedial)

# Others

schtype_others <- my_data_3 %>%
  group_by(sch_type, mathslessons_others) %>%
  summarise(count = n_distinct(CNTSCHID)) %>%
  mutate(percentage = count / sum(count) * 100) %>%
  pivot_wider(names_from = mathslessons_others, values_from = c(count, percentage), values_fill = 0)
...
avg_time_spent <- my_data_3 %>%
  distinct(CNTSCHID, .keep_all = TRUE) %>%
  summarise(avg_time_spent_in_class = mean(minutes_spent_in_class))

print(avg_time_spent)

# Calculate the average time spent in class by school type

avg_time_sch <- my_data_3 %>%
  distinct(CNTSCHID, .keep_all = TRUE) %>%
  group_by(sch_type) %>%
...
# Filter to get distinct CNTSCHID

distinct_data <- my_data_3 %>%
  distinct(CNTSCHID, .keep_all = TRUE)

ggplot(distinct_data, aes(x = sch_type, y = minutes_spent_in_class, col = sch_type)) +
  geom_boxplot() +
  labs(x = "School Type", y = "Time Spent in Class (mins)") +
  ggtitle("Average time spent by teachers in mathematics class") +
...
sch_inv_spe <- my_data_3 %>%
  group_by(pd_SchInviteSpecialist) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
sch_org_wor <- my_data_3 %>%
  group_by(pd_SchOrganiseWorkshop) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
sch_org_spe_wor <- my_data_3 %>%
  group_by(pd_SchOrganiseSpecificWorkshop) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
math_content <- my_data_3 %>%
  group_by(sch_offer_pd_MathsContent) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
math_pedagogy <- my_data_3 %>%
  group_by(sch_offer_pd_MathsPedagogy) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
math_curriculum <- my_data_3 %>%
  group_by(sch_offer_pd_MathsCurriculum) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
math_int_digital <- my_data_3 %>%
  group_by(sch_offer_pd_IntegrateDigitalResources) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
math_imp_cri_thi <- my_data_3 %>%
  group_by(sch_offer_pd_ImproveStudentCriticalThinking) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
math_assessment <- my_data_3 %>%
  group_by(sch_offer_pd_MathsAssessment) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
math_add_stu_needs <- my_data_3 %>%
  group_by(sch_offer_pd_AddressStudentsNeeds) %>%
  summarise(n = n_distinct(CNTSCHID)) %>%
  mutate(proportion = n / sum(n))
...
# Calculate the average commitment to professional development by school type

avg_comm_sch <- my_data_3 %>%
  distinct(CNTSCHID, .keep_all = TRUE) %>%
  group_by(sch_type) %>%
  summarise(avg_comm_ = mean(percent_tchAttendPD))

# Calculate the average time spent in class by school type

avg_comm_PD <- my_data_3 %>%
  distinct(CNTSCHID, .keep_all = TRUE) %>%
  summarise(avg_commPD_ = mean(percent_tchAttendPD))
...
for (i in 1:length(pv_cols)) {
  ...
  empty_models[[i]] <- lmer(formula, data = my_data_3)
}
...
for(i in 1:10) {
  ...
  models_obj5[[i]] <- lmer(get(pv_name) ~ ft_maths_tchs + (1|sch_type), data = my_data_3)
}
...
for(i in 1:10) {
  pv_name <- paste0("PV", i, "MATH")
  models_obj6[[i]] <- lmer(get(pv_name) ~ pd_SchInviteSpecialist + pd_SchOrganiseWorkshop + pd_SchOrganiseSpecificWorkshop + (1|sch_type), data = my_data_3)
}
...
for(i in 1:10) {
  pv_name <- paste0("PV", i, "MATH")
  models_obj7[[i]] <- lmer(get(pv_name) ~ ft_lvl6_tch + ft_lvl5_tch + (1|sch_type), data = my_data_3)
}
